...
for model in app_models:
    ...
    urlpatterns.append(url(regex + r'change/(?P<pk>\d+)/$', views.DataChangeView.as_view(model = model, template_name = 'datenmanagement/dataform.html', success_url = reverse_lazy('datenmanagement:' + model_name + 'list')), name = model_name + 'change'))
    urlpatterns.append(url(regex + r'delete/(?P<pk>\d+)/$', views.DataDeleteView.as_view(model = model, template_name = 'datenmanagement/datadelete.html', success_url = reverse_lazy('datenmanagement:' + model_name + 'list')), name = model_name + 'delete'))
