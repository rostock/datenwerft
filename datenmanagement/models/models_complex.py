...
class Baugrunduntersuchungen_Dokumente(ComplexModel):
  ...
  pdf = FileField(
    ...
    storage=OverwriteStorage(),
    upload_to=str(path_and_rename(settings.PDF_PATH_PREFIX_PRIVATE + 'baugrunduntersuchungen')),
    max_length=255,
  )
...
class Baustellen_Fotodokumentation_Baustellen(ComplexModel):
  ...
  def __str__(self):
    return str(self.bezeichnung) + (' [Straße: ' + str(self.strasse) + ']' if self.strasse else '')
...
class Baustellen_geplant_Dokumente(ComplexModel):
  ...
  pdf = FileField(
    verbose_name='Dokument',
    storage=OverwriteStorage(),
    upload_to=str(path_and_rename(settings.PDF_PATH_PREFIX_PUBLIC + 'baustellen_geplant')),
    max_length=255,
  )
  ...
  def __str__(self):
    return str(self.baustelle_geplant) + ' mit Bezeichnung ' + str(self.bezeichnung)
...
class Feuerwehrzufahrten_Feuerwehrzufahrten(ComplexModel):
  ...
  def __str__(self):
    if self.registriernummer:
      ...
      return 'ohne Registriernummer – ' + str(self.bemerkungen)
    else:
...
class Freizeitsport(ComplexModel):
  ...
  def string_representation(self):
    ...
    bezeichnung_str = str(self.bezeichnung) + ', ' if self.bezeichnung else ''
...
class Haltestellenkataster_Haltestellen(ComplexModel):
  ...
  def __str__(self):
    return (
      ...
      + ' [ID: '
      + str(self.id)
      + (', HAFAS-ID: ' + str(self.hst_hafas_id) if self.hst_hafas_id else '')
      + (
        ', Bus-/Bahnsteig: ' + str(self.hst_bus_bahnsteigbezeichnung)
...
class Punktwolken_Projekte(ComplexModel):
  ...
  def save(
    ...
    for element in Punktwolken.objects.filter(projekt=self):
      logger.info(f'Updating Punktwolke {element}')
      element.save()
    super().save(
      force_insert=force_insert,
      force_update=force_update,
  ...
  def save(
    ...
    if not self.vcp_object_key and is_broker_available():
      ...
      if not self.geometrie:
        # run function asynchronously with rq
      ...
      enqueue(
        ...
        objectkey=f'/dataset/{self.dateiname}',
      )
...
class Strassen_Simple(ComplexModel):
  ...
  def __str__(self):
    return str(self.bezeichnung) + ' (' + str(self.schluessel) + ')'
