...
from bemas.utils import (
  get_complaint_status_change_deadline_date,
  get_orphaned_organizations,
  get_orphaned_originators,
  get_orphaned_persons,
)
...
class IndexView(TemplateView):
  ...
  def get_context_data(self, **kwargs):
    ...
    figure = (
      Complaint.objects.filter(status=Status.get_closed_status())
      .filter(status_updated_at__gte=one_year_ago)
      .count()
    )
    statistics.append(
      {
    ...
    figure = (
      Complaint.objects.filter(status=Status.get_closed_status())
      .filter(status_updated_at__gte=one_month_ago)
      .count()
    )
    statistics.append(
      {
...
class MapView(TemplateView):
  ...
  def get_context_data(self, **kwargs):
    ...
    context['complaints_types_of_immission'] = list(
      Complaint.objects.order_by('type_of_immission')
      .values_list('type_of_immission__title', flat=True)
      .distinct()
    )
    complaints_originators = (
      Complaint.objects.order_by('originator').values_list('originator', flat=True).distinct()
    )
    ...
    if Originator.objects.filter(operator_organization__isnull=False):
      context['originators_operators_organizations'] = list(
        Originator.objects.filter(operator_organization__isnull=False)
        .order_by('operator_organization')
        .values_list('operator_organization__name', flat=True)
        .distinct()
      )
    if Originator.objects.filter(operator_person__isnull=False):
      originators_operators_persons_ids = (
        Originator.objects.filter(operator_person__isnull=False)
        .values('operator_person')
        .distinct()
      )
    ...
    context['objects_count'] = get_model_objects(Complaint, True) + get_model_objects(
      Originator, True
    )
