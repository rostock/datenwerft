...
from d3.models import Metadaten, Vorgang, VorgangMetadaten
...
class FetchMetaDataRequestView(JsonView):
  def get(self, request, *args, **kwargs):
    ...
    if not process_id:
      return JsonResponse({'error': "Missing 'processId' parameter."}, status=400)
    ...
    html = render_to_string(
      'd3/metadata_detail.html',
      {
        'random_uuid': str(uuid.uuid4()),
        'file_list_url': resolve_url(
          'd3:' + str(model_name) + '_d3_list_file', model_id, vorgang.id
        ),
        'model_name': model_name,
        'model_id': model_id,
        'vorgangs_id': vorgang.id,
        'metadata': [{'titel': pm.metadaten.titel, 'wert': pm.wert} for pm in process_metadata],
      },
    )
...
class ListeDateienView(View):
  ...
  def __init__(self, datenmanagement_model=None, *args, **kwargs):
    ...
    self.content_type_id = ContentType.objects.get(
      app_label='datenmanagement', model=datenmanagement_model.lower()
    ).id
...
class DokumentenView:
  ...
  def render_view(self, request, form, object_id: str):
    view_params = {
      'url_back': reverse(
        'datenmanagement:' + self.datenmanagement_model + '_change', args=[object_id]
      ),
      'form': form,
    }
