...
class GenericCreateView(CreateView):
  template_name = 'angebotsdb/form.html'
  # model = None
  success_url = None
...
class GenericUpdateView(UpdateView):
    ...
    if is_service_model:
      ...
      # ── Published: Draft suchen oder Formular normal öffnen ─────────────
        ...
        if draft is not None:
          ...
          draft_url = reverse(
            f'angebotsdb:{self.model.__name__.lower()}_update',
            args=[draft.pk],
          )
  ...
  def form_valid(self, form):
    ...
    if getattr(self, '_form_locked', False):
      raise PermissionDenied('Dieses Formular ist gesperrt und kann nicht gespeichert werden.')
  ...
  def get_context_data(self, **kwargs):
    ...
    if is_service_model:
      ...
      context['revision_mailto_links'] = {
        field_name: build_mailto(
          reviewer_info, service_name, field_labels.get(field_name, field_name), comment
        )
...
class GenericDeleteView(DeleteView):
  template_name = 'angebotsdb/delete.html'
  # model = None
  success_url = None
