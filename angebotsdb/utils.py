...
def get_service_instance(service_type: str, service_id: int):
  ...
  try:
    model = apps.get_model('angebotsdb', service_type)
...
def get_user_provider(user):
  """
  Returns the provider associated with the user's AngebotsDB profile,
...
def get_user_org_unit(user):
  """
  Returns the organisational unit associated with the user's AngebotsDB profile,
...
def authorized_to_review(user, service=None):
  ...
  - Superusers and AngebotDB admins can always review.
...
def authorized_to_edit(user, service=None):
  ...
  - Superusers and AngebotsDB admins can always edit.
...
def is_angebotsdb_admin(user):
  """
  checks if passed user is a AngebotsDB admin

  :param user: user
  :return: passed user is a AngebotsDb admin?
  """
...
def is_angebotsdb_user(user, only_angebotsdb_user_check=False):
  """
  checks if passed user is a AngebotsDB user
  (and optionally checks if it is a AngebotsDB user only)

  :param user: user
  :param only_angebotsdb_user_check: check if user is a AngebotsDB user only?
  :return: passed user is a AngebotsDB user (only)?
  """
  in_angebotsdb_groups = user.groups.filter(name__in=[ADMIN_GROUP, USERS_GROUP])
  if in_angebotsdb_groups:
    if only_angebotsdb_user_check:
      # if user is a KIJU user only, he is not a member of any other group
      return in_angebotsdb_groups.count() == user.groups.all().count()
    else:
      return True
  return False
...
def build_mailto(
  reviewer_info: dict, service_name: str, field_label: str, comment: str
) -> str | None:
  """
